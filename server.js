...
// Twilio media URLs require your Twilio credentials to download (they're not public).
...
// Transcribes a WhatsApp voice note using OpenAI's Whisper model.
// Handles English and Nigerian-accented English/Pidgin well; expect lower
// accuracy on pure Yoruba/Igbo/Hausa audio — that's a real limitation of
// Whisper today, not a bug in this code.
...
// Describes a WhatsApp image using OpenAI's vision-capable model, and pulls
// out any long digit sequence found (account number, BVN, NIN, amount) so it
// can be reused as if the person had typed it.
async function describeImage(mediaUrl) {
      ...
      messages: [
        {
          role: "user",
          content: [
            { type: "text", text: "Describe this image briefly, and separately list any numbers (account numbers, amounts, BVN, NIN, phone numbers) you can read in it." },
  ...
  const digitMatch = description.match(/\d{6,}/);
  return { description, extractedNumber: digitMatch ? digitMatch[0] : null };
}

app.post("/api/whatsapp", async (req, res) => {
  let incomingMessage = (req.body.Body || "").trim();
  ...
  if (numMedia > 0) {
    ...
    try {
      if (mediaType.startsWith("audio")) {
        incomingMessage = (await transcribeVoiceNote(mediaUrl)).trim();
        console.log("VOICE NOTE TRANSCRIBED:", incomingMessage);
      } else if (mediaType.startsWith("image")) {
        const { description, extractedNumber } = await describeImage(mediaUrl);
        console.log("IMAGE DESCRIBED:", { description, extractedNumber });
        if (extractedNumber) {
          twiml.message(description);
          incomingMessage = extractedNumber;
        } else {
          twiml.message(description);
